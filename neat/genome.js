...
export default class Genome {
  ...
  initialize() {
    ...
    for (let i = 0; i < outputNodes.length; i++) {
      let weight = this.randomDoubleFromInterval(
        -CONFIG.NEW_WEIGHT_RANGE,
        CONFIG.NEW_WEIGHT_RANGE
      );
  ...
  getOutputNodes() {
    return this.nodes.slice(
      ...
      1 + this.numInputNodes + this.numOutputNodes
    );
  }
  ...
  addConnection() {
    ...
    while (!connectionOk) {
      ...
      var node2 = this.nodes[
        Math.floor(
          Math.random() * (this.getHiddenNodes().length + this.numOutputNodes)
        ) +
          this.numInputNodes +
          1
      ];
    ...
    this.connections.push(
      new Connection(
        ...
        weight,
        true,
        Innovation.getNextInnovationNumber(node1.id, node2.id)
      )
    );
  }
  ...
  isFullyConnected() {
    ...
    if (!CONFIG.ALLOW_LOOPS) {
      const numConnectionsFromInput =
        this.numInputNodes * (numHiddenNodes + this.numOutputNodes);
      const numConnectionsFromHidden =
        numHiddenNodes * (numHiddenNodes - 1 + this.numOutputNodes);
      const numConnectionsFromBias = numHiddenNodes + this.numOutputNodes;
      sum =
        numConnectionsFromInput +
        numConnectionsFromHidden +
        numConnectionsFromBias;
    } else {
      const numConnectionsFromInput =
        this.numInputNodes * (numHiddenNodes + this.numOutputNodes);
      const numConnectionsFromHidden =
        numHiddenNodes * (numHiddenNodes + this.numOutputNodes);
      const numConnectionsFromBias = numHiddenNodes + this.numOutputNodes;
      const numConnectionsFromOutput =
        this.numOutputNodes * (numHiddenNodes + this.numOutputNodes);
      sum =
        numConnectionsFromInput +
        numConnectionsFromHidden +
        numConnectionsFromBias +
        numConnectionsFromOutput;
    }
  ...
  alreadyConnected(node1, node2) {
    for (let i = 0; i < this.connections.length; i++) {
      if (
        this.connections[i].start == node1.id &&
        this.connections[i].end == node2.id
      ) {
        return true;
      }
    }
  ...
  sortNodes(testConnection = null) {
    ...
    let connections = JSON.parse(
      JSON.stringify(this.connections.filter(c => c.enabled))
    );
