...
export default class NEAT {
  ...
  repopulate() {
    ...
    for (let i = 0, len = this.population.length; i < len; i++) {
      ...
      if (!speciesFound) {
        const newSpeciesId =
          Math.max(...Object.keys(this.speciesDict).map(id => +id)) + 1;
        this.speciesDict[newSpeciesId] = new Species(
          newSpeciesId,
    ...
    for (let key in this.speciesDict) {
      const species = this.speciesDict[key];
      species.numOffsprings = Math.floor(
        (species.fitnessSum / totalFitnessSum) * this.populationSize
      );
