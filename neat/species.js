...
export default class Species {
  ...
  getChampion() {
    ...
    if (
      this.champion &&
      Math.abs(this.champion.fitness - this.organisms[0].fitness) <
  ...
  isCompatibleWith(player) {
    ...
    for (let i = 0, len = innovationSet.length; i < len; i++) {
      if (
        this.representative.brain.connections.some(
          c => c.innovation == innovationSet[i]
        ) &&
        player.brain.connections.some(c => c.innovation == innovationSet[i])
      ) {
        sumWeightDifferenceMatchingGenes += Math.abs(
          this.representative.brain.connections.find(
            c => c.innovation == innovationSet[i]
          ).weight -
            player.brain.connections.find(c => c.innovation == innovationSet[i])
              .weight
        );
        numMatchingGenes++;
      } else {
    ...
    const avgWeightDifferenceMatchingGenes =
      numMatchingGenes > 0
        ? sumWeightDifferenceMatchingGenes / numMatchingGenes
        : 0.0;
    const N =
      this.representative.brain.connections.length > 19 ||
      player.brain.connections.length > 19
        ? Math.max([
            this.representative.brain.connections.length,
            player.brain.connections.length
          ])
        : 1.0;

    return (
      (CONFIG.C1 * numExcessAndDisjointGenes) / N +
  ...
  cull() {
    ...
    this.organisms = this.organisms.slice(
      0,
      Math.floor(this.organisms.length / 2.0)
    );
  }
  ...
  mutate(parent) {
    ...
    let disabledConenctions = offspring.brain.connections.filter(
      c => c.enabled == false
    );
  ...
  crossover(parent1, parent2) {
    ...
    const nodesSet = [
      ...new Set([
        ...parent1.brain.nodes.map(n => n.id),
        ...parent2.brain.nodes.map(n => n.id)
      ])
    ];
    for (let i = 0, len = nodesSet.length; i < len; i++) {
      if (
        parent1.brain.nodes.some(n => n.id == nodesSet[i]) &&
        parent2.brain.nodes.some(n => n.id == nodesSet[i])
      ) {
        if (Math.random() < 0.5) {
          brainOffspring.nodes.push(
            parent1.brain.nodes.find(n => n.id == [nodesSet[i]]).clone()
          );
        } else {
          brainOffspring.nodes.push(
            parent2.brain.nodes.find(n => n.id == [nodesSet[i]]).clone()
          );
        }
      }
    }
      ...
      if (
        ...
        if (Math.random() < 0.5) {
          brainOffspring.connections.push(
            parent1.brain.connections
              .find(c => c.innovation == innovationSet[i])
              .clone()
          );
        } else {
          brainOffspring.connections.push(
            parent2.brain.connections
              .find(c => c.innovation == innovationSet[i])
              .clone()
          );
        }
    ...
    const neededNodesSet = [
      ...new Set([
        ...brainOffspring.connections.map(c => c.start),
        ...brainOffspring.connections.map(c => c.end)
      ])
    ];
    for (let i = 0, len = neededNodesSet.length; i < len; i++) {
      if (!brainOffspring.nodes.find(n => n.id == neededNodesSet[i])) {
        if (parent1.brain.nodes.some(n => n.id == neededNodesSet[i])) {
          brainOffspring.nodes.push(
            parent1.brain.nodes.find(n => n.id == [neededNodesSet[i]]).clone()
          );
        } else {
          brainOffspring.nodes.push(
            parent2.brain.nodes.find(n => n.id == [neededNodesSet[i]]).clone()
          );
        }
      }
    }
